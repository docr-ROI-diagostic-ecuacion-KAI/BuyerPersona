const styleId = "docroi-hero-tools-layout-polish-style";
...
function fixHeroText() {
  document.querySelectorAll<HTMLElement>(".objective-panel dt").forEach((node) => {
    if (/harramienta|herramienta/i.test(node.textContent || "")) node.textContent = "Herramientas";
  });
}
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  if (root) new MutationObserver(applyPolish).observe(root, { childList: true, subtree: true, characterData: true });
});
