...
function installClustersStyles() {
  ...
  style.textContent = `
    .docroi-topic-clusters-step .docroi-cluster-builder { order: 1; }
    ...
    @media (max-width: 900px) {
      .docroi-cluster-branches,
      .docroi-cluster-questions { grid-template-columns: 1fr; }
    }
  `;
...
function signatureFromData() {
  ...
  return JSON.stringify({ product: data.product, persona: data.fictionalName, pains: derivePains(data), gains: deriveGains(data), trust: data.trustTrigger });
}
...
function getClusterModel() {
  ...
  const branches = [
    { title: clusters[0]?.subtopics?.[0] || "Criterios de eleccion", keyword: kw.informational?.[0] || `que es ${product}`, body: pains[0] || "Sale del problema, la duda o la barrera que el Buyer Persona necesita resolver antes de confiar." },
    { title: clusters[0]?.subtopics?.[2] || "Comparativa y alternativas", keyword: kw.comparative?.[0] || `mejor ${product}`, body: gains[0] || "Sale del beneficio esperado y permite crear contenido que compare opciones con criterio." },
    { title: clusters[1]?.subtopics?.[1] || "Prueba, demo o decision", keyword: kw.transactional?.[1] || `solicitar demo ${product}`, body: data.trustTrigger || `Sale de la evidencia que ${persona} necesita para pasar de interes a accion.` },
  ];
  return { data, pillar, branches };
}

function clusterBuilderHtml(signature: string) {
  const { data, pillar, branches } = getClusterModel();
  const product = data.product || "producto";
  return `
    <div class="docroi-cluster-builder" data-docroi-cluster-builder="1" data-docroi-signature="${escapeHtml(signature)}">
...
function enhanceClusters() {
  ...
  if (formGrid && existingBuilder?.dataset.docroiSignature !== signature) {
    const html = clusterBuilderHtml(signature);
  ...
  if (frame && !document.querySelector('[data-docroi-clusters-feed="1"]')) {
    frame.insertAdjacentHTML("beforeend", clustersFeedHtml());
  }
}
